...
# Função para encontrar o caminho usando backtracking
def find_path_backtracking(current_pos, exit_pos, visited, last_direction=None):
    ...
    # Nova ordem de movimento: direita, esquerda, baixo e cima
    directions = [(0, 1), (0, -1), (1, 0), (-1, 0)]

    for direction in directions:
        # Ignorar a direção oposta à última para evitar voltar imediatamente
    ...
    # Se não encontrar caminho nas direções padrão, retorne ao canto inicial
    if (row, col) != (0, 0):
        # Ignorar a direção oposta à última para evitar voltar imediatamente
        if last_direction is not None:
            ...
            path_to_corner = find_path_backtracking((0, 0), exit_pos, visited.copy())
